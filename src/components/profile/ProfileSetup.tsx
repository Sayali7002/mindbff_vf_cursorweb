...
const stepIcons: { [key: string]: string } = {
  ...
  'gender': 'wc',
  'supportType': 'support_agent',
  'journeyPreferences': 'psychology',
...
const additionalSteps: Step[] = [
  {
    ...
    fields: [
      { 
        ...
        type: 'radio', 
        options: ['I want to be there for others', 'I want to vent out'],
        required: false, 
        category: 'optional' 
      },
      { 
  ...
  const handleInputChange = (name: string, value: string | boolean | string[]) => {
    setProfileData(prev => {
      ...
      if (name === 'supportType') {
        ...
        } else if (value === 'I want to vent out') {
            ...
            {field.options?.map((option, i) => (
              <label key={i} className="flex items-center space-x-3 cursor-pointer">
                <input
                  ...
                  name={field.name}
                  value={option}
                  checked={field.name === 'supportType' ? (option === 'I want to be there for others' ? profileData.supportType === 'support-giver' : profileData.supportType === 'support-seeker') : profileData[field.name as keyof UserProfile] === option}
                  onChange={(e) => {
                    const currentType = profileData.supportType || '';
                    const newType = e.target.checked
                      ? option === 'I want to be there for others' ? 'support-giver' : 'support-seeker'
        ...
        {showOptionalSteps && !isMandatoryStep ? (
          ...
          >
            <h1 className="text-3xl font-bold text-gray-900 flex items-center justify-center">
              <span className="material-icons mr-3 text-blue-600 text-4xl">
                {stepIcons[currentStepData.id] || 'settings'}
              </span>
      ...
      {showCelebrationModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full text-center">
            ...
            <h2 className="text-2xl font-bold text-blue-600 mb-4">Welcome to MindBFF!</h2>
