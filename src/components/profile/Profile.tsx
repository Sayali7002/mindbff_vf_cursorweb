    ...
    async function fetchProfile() {
      ...
      try {
        ...
        if (session) {
          ...
          let profile = await fetchAndUpdateUserProfile(session.user.id);
          
          if (profile) {
            console.log("Profile fetched successfully:", profile);
            console.log("Profile completedSetup status:", profile.completedSetup);
                   // Decrypt sensitive fields
                   profile = decryptProfileFields(profile, session.user.id); 
  ...
  const sections = [
    ...
    },
    {
      ...
      fields: [
       ...
       { name: 'supportType', label: 'Support Type', type: 'select', options: ['I want to vent out', 'I want to be there for others'], required: false }, 
    ...
    if (!isEditing) {
      if (field.name === 'supportPreferences' && Array.isArray(profileData.supportPreferences)) {
        ...
        if (profileData.supportType === 'support-seeker') {
          return <p className="text-gray-600">I want to vent out</p>;
        } else if (profileData.supportType === 'support-giver') {
          return <p className="text-gray-600">I want to be there for others</p>;
        } else {
        ...
        if (field.name === 'supportType') {
          return (
            ...
            >
              ...
              <option value="support-seeker">I want to vent out</option>
              <option value="support-giver">I want to be there for others</option>
            </select>
          );
        } else {
          return (
