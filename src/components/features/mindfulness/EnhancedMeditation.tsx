...
export default function EnhancedMeditation({ onClose, currentResource }: EnhancedMeditationProps) {
  ...
  return (
    <div className="bg-white rounded-xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
      ...
      <div className="mt-6">
        <button
          className="flex items-center gap-2 text-blue-800 font-medium focus:outline-none mb-2"
          onClick={() => setShowGuidance((v) => !v)}
          aria-expanded={showGuidance}
          aria-controls="meditation-guidance-section"
        >
          <span>Meditation Guidance</span>
          <span className={`material-icons transition-transform duration-200 ${showGuidance ? 'rotate-180' : ''}`}>expand_more </span>
        </button>
        <div
          id="meditation-guidance-section"
          className={`overflow-hidden transition-all duration-300 ${showGuidance ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}
          style={{ willChange: 'max-height, opacity' }}
        >
          <div className="bg-blue-50 p-4 rounded-lg text-sm mt-1">
