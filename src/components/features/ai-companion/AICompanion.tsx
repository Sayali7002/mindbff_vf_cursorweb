...
const personalities = [
  { key: 'default', label: 'Default Therapist', desc: 'Empathetic, supportive' },
  { key: 'vivekananda', label: 'Swami Vivekananda', desc: 'Spiritual, motivational' },
  { key: 'kalam', label: 'Dr. APJ Abdul Kalam', desc: 'Scientific, inspirational' },
  { key: 'sadhguru', label: 'Sadhguru', desc: 'Contemporary, spiritual' },
  { key: 'krishnamurti', label: 'J. Krishnamurti', desc: 'Philosophical, introspective' },
]
...
export default function AICompanion() {
  ...
  const handleSendMessage = async (message: string) => {
    ...
    // Message limit logic
    let aiMessageCount = parseInt(localStorage.getItem('aiMessageCount') || '0', 10);
    if (aiMessageCount >= MAX_AI_MESSAGES) {
      setError('You have reached the maximum number of messages allowed. Please contact mindbffglobal@gmail.com for subscription plans.');
      return;
    }
    localStorage.setItem('aiMessageCount', String(aiMessageCount + 1));
    ...
    try {
      ...
      try {
        const response = await fetch('/api/chat', {
          ...
          body: JSON.stringify({
            ...
            personality: selectedPersonality !== 'default' ? selectedPersonality : undefined
          }),
        })
        if (!response.ok) {
        ...
        aiIsCrisis = data.isCrisis;
      } catch (apiError: any) {
        aiResponse = "I'm sorry, I'm having trouble connecting to my services. Please try again in a moment or contact support if the issue persists.";
        setError(apiError.message || 'Failed to get response from AI');
      }
      // Wait exactly 1 second before showing the AI response
      ...
      if (aiResponse) {
        ...
        setMessages(prev => {
          ...
          speakText(aiResponse, newMessages.length - 1);
          if (aiIsCrisis) setShowCrisisResources(true);
          return newMessages;
        });
      }
    } catch (error: any) {
  ...
  const toggleListening = () => {
    ...
    if (isMobileChrome()) {
      setError('Speech recognition is not supported on mobile Chrome. Please use a different browser or device.');
      return;
    }
    try {
      if (isListening) {
  ...
  const toggleAnonymousMode = () => {
    if (!isAnonymous && messages.length > 1) {
      ...
      if (window.confirm("Enabling anonymous mode will clear your current chat history. Continue?")) {
        setMessages([{
          role: 'assistant',
          content: "Hello! I'm your MindBFF companion. I'm here to listen and support you. How are you feeling today?",
          timestamp: new Date().toISOString()
        }]);
        setIsAnonymous(true);
      }
  ...
  return (
    <div className="flex flex-col h-full w-full">
      ...
      <div className="flex flex-row items-center justify-between gap-4 py-2 px-4 bg-[#ffe4e6] bg-opacity-20 border-b border-[#ffe4e6] border-opacity-30">
        <label className="relative inline-flex items-center cursor-pointer mr-4">
          ...
          />
          <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-[#ffe4e6] rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#ffe4e6]"></div>
          <span className="ml-3 text-sm font-medium text-gray-700">Anonymous</span>
        </label>
        <div className="relative" ref={dropdownRef}>
          <button
            type="button"
            className="w-55 bg-white border border-gray-300 rounded-lg shadow-sm px-2 py-2 text-left focus:outline-none focus:ring-2 focus:ring-blue-400 flex items-center justify-between"
            onClick={() => setDropdownOpen((open) => !open)}
          >
            <span className="text-sm font-medium text-gray-700">{personalities.find(p => p.key === selectedPersonality)?.label}</span>
            <span className="material-icons ml-2 text-gray-400 text-base">expand_more</span>
          </button>
          {dropdownOpen && (
            <div className="absolute z-10 mt-2 w-50 rounded-lg bg-white shadow-lg border border-gray-200 max-h-60 overflow-auto animate-fade-in">
              {personalities.map(p => (
                <div
                  key={p.key}
                  onClick={() => { setSelectedPersonality(p.key); setDropdownOpen(false); }}
                  className={`px-4 py-2 cursor-pointer hover:bg-blue-50 ${selectedPersonality === p.key ? 'bg-blue-100' : ''}`}
                >
                  <div className="font-medium text-gray-700 text-sm">{p.label}</div>
                  <div className="text-xs italic text-gray-500 mt-0.5">{p.desc}</div>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="bg-blue-50 bg-opacity-5 py-2 px-4 border-b border-[#ffe4e6] border-opacity-20">
      ...
      {/* Main chat area with full screen styling */}
       ...
       <div className="flex-1 overflow-y-auto px-2 py-4 pb-20" style={{ minHeight: 0 }}>
            ...
            >
              <div
                className={`max-w-[80%] rounded-lg p-4 relative ${
                  message.role === 'user'
                    ? 'bg-[#ffe4e6] text-gray-800 shadow'
                    : 'bg-[#ede9fe] text-gray-800 shadow'
                }`}
              >
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-4 shadow-lg">
        <div className="max-w-4xl mx-auto">
              ...
              <div className="flex space-x-2">
                {availableQuickResponses.map((response, index) => (
                  <button
                    ...
                    className="bg-[#ffe4e6] bg-opacity-25 hover:bg-opacity-50 whitespace-nowrap px-4 py-2 rounded-full text-sm text-gray-700"
                  >
                    {response}
                  </button>
                ))}
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              <span className="material-icons text-[#a855f7]">{isListening ? 'mic' : 'mic_none'}</span>
            </button>
            <input
              type="text"
              value={inputText}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            ...
            <div className="space-y-4 text-gray-700">
              <p>
                Sign up to access it anytime
              </p>
              <p>
                Creating an account also gives you access to all other features including Peer Support, and Mindfulness Tools.
              </p>
            </div>
            <div className="flex space-x-3 mt-6">
              ...
              <Button
                onClick={navigateToSignup}
                className="flex-1 bg-[#ffe4e6] hover:bg-[#f0c0b0] text-gray-800"
              >
                Sign Up
              </Button>
            </div>
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div className="bg-white rounded-xl max-w-md w-full p-6 space-y-6 max-h-[90vh] overflow-y-auto relative"> {/* Added max-h and overflow-y */}
      <div className="flex justify-between items-center sticky top-0 bg-white pb-4 z-10"> {/* Added sticky for header */}
        <h3 className="text-xl font-bold text-[#a855f7]">Crisis Resources</h3>
        <button
          ...
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            ...
            <div className="space-y-4 text-gray-700">
              ...
              <p className="font-medium">
                By default, you're using anonymous mode. Your chat history will not be saved after you leave this page.
              </p>
            </div>
            <Button
