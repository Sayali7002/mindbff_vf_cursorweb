...
import { getUserKey, encryptMessage, decryptMessage, isEncrypted } from './encryption';
...
export async function createMindfulnessEntry(entry: Omit<MindfulnessEntry, 'id' | 'created_at' | 'updated_at'>, authenticatedClient?: any) {
  ...
  // Encrypt content
  const userKey = getUserKey(entry.user_id);
  const encryptedContent = encryptMessage(entry.content, userKey);
  ...
  if (data && data.content && isEncrypted(data.content)) {
    try {
      data.content = decryptMessage(data.content, userKey);
    } catch (e) {
      data.content = '[Unable to decrypt]';
    }
  }
...
export async function getMindfulnessEntries(userId: string, type?: MindfulnessEntryType, authenticatedClient?: any) {
  ...
  const decrypted = (data as MindfulnessEntry[]).map(entry => {
