...
export default function PeerDetailPage({ params }: { params: { id: string } }) {
  ...
  // Get the current request status for this peer
  const requestStatus = peer ? getRequestStatus(peer.id) : 'none';
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6">
            <div className="flex items-start space-x-4">
              <div className="relative">
                ...
                {peer.isActive && (
                  <span className="absolute bottom-0 right-0 h-4 w-4 bg-green-600 rounded-full border-2 border-white"></span>
                )}
              </div>
              <div>
