...
export default function PeerSupportPage() {
  ...
  const ENABLE_PEER_FETCHING_FOR_SUPPORT_GIVERS = true;
  ...
  // Set default tab based on user type - Sent first for support-seekers, Received first for support-givers
  const [requestsTab, setRequestsTab] = useState<'sent' | 'received'>('sent');
  ...
  useEffect(() => {
    ...
    if (userProfile) {
      ...
      if (userProfile.supportType === 'support-seeker') {
        ...
        // Check configuration flag for support-givers
        if (!ENABLE_PEER_FETCHING_FOR_SUPPORT_GIVERS) {
          console.log('Peer fetching disabled for support-givers by configuration flag');
          return; // Don't fetch peers if flag is disabled
        }
  ...
  // Update default requests tab based on user type
  useEffect(() => {
    if (userProfile?.supportType === 'support-giver') {
      setRequestsTab('received');
    } else if (userProfile?.supportType === 'support-seeker') {
      setRequestsTab('sent');
    }
  }, [userProfile?.supportType]);
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {isAuthenticated && userProfile && (
          <div className="bg-white p-4 rounded-xl shadow-sm">
            ...
            {/* Concise and impactful line below "I want to" section */}
            <div className="mt-3 pt-3 border-t border-gray-100">
              <p className="text-sm text-gray-600 italic">
                {userProfile.supportType === 'support-seeker' 
                  ? "You're strong. This safe space is here for you in every step towards healing." 
                  : userProfile.supportType === 'support-giver'
                  ? "Your experience can change someone's life today. Make a difference!"
                  : "Every connection matters in the journey to wellness. You are strong!"
                }
              </p>
            </div>
                </div>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                ...
                <div className="flex mt-2">
                  {/* Show Sent first for support-seekers, Received first for support-givers */}
                  {userProfile?.supportType === 'support-seeker' ? (
                    <>
                      <button
        ...
        {/* Filters - Only show if peer fetching is enabled for support-givers */}
        {!(userProfile?.supportType === 'support-giver' && !ENABLE_PEER_FETCHING_FOR_SUPPORT_GIVERS) && (
          <div className="bg-white p-4 rounded-xl shadow-sm">
            <div className="flex justify-between items-center">
          ...
          {/* No Peers Available - Only show if peer fetching is enabled for support-givers */}
          {!(userProfile?.supportType === 'support-giver' && !ENABLE_PEER_FETCHING_FOR_SUPPORT_GIVERS) && filteredPeers.length === 0 && (
          ...
          {/* Peer Cards - Only show if peer fetching is enabled for support-givers */}
          {!(userProfile?.supportType === 'support-giver' && !ENABLE_PEER_FETCHING_FOR_SUPPORT_GIVERS) && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <PeerCard
                ...
                requestStatus={getRequestStatus(peer.id)}
                isSubmitting={isSendingRequest}
              />
                        ))}
                      </div>
