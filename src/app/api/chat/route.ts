...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      // Build personality context if provided
      let personalityContext = ''
      if (personality) {
        // You can expand this switch for more personalities
        switch (personality) {
          case 'vivekananda':
            personalityContext = `Respond in the style of Swami Vivekananda, focusing on youth motivation, spirituality, and Indian values. Use inspiring, energetic, and compassionate language.`
            break
          case 'kalam':
            personalityContext = `Respond in the style of Dr. APJ Abdul Kalam, focusing on science, dreams, optimism, and practical wisdom for youth.`
            break
          case 'sadhguru':
            personalityContext = `Respond in the style of Sadhguru, blending contemporary spiritual wisdom, clarity, and practical advice for young people.`
            break
          case 'krishnamurti':
            personalityContext = `Respond in the style of J. Krishnamurti, with philosophical depth, introspection, and encouragement for self-inquiry.`
            break
          default:
            personalityContext = personality // fallback: use as custom instruction
        }
      }

      // Build conversation history
      const history = messages.map((msg: any) => `${msg.role === 'user' ? 'User' : 'Assistant'}: ${msg.content}`).join('\n')
